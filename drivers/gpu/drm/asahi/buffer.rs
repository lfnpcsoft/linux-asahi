...
pub(crate) struct Buffer {
    inner: Ref<Mutex<BufferInner::ver>>,
}
...
impl Scene::ver {
    ...
    pub(crate) fn gpu_pointer<'a>(&'a self) -> GPUPointer<'a, buffer::Scene::ver> {
    ...
    pub(crate) fn tvb_heapmeta_pointer<'a>(&'a self) -> GPUPointer<'a, &'a [u8]> {
        self.object.tvb_heapmeta.gpu_pointer()
    }

    pub(crate) fn tvb_tilemap_pointer<'a>(&'a self) -> GPUPointer<'a, &'a [u8]> {
        self.object.tvb_tilemap.gpu_pointer()
    }

    pub(crate) fn preempt_buf_pointer<'a>(&'a self) -> GPUPointer<'a, buffer::PreemptBuffer> {
        self.object.preempt_buf.gpu_pointer()
    }

    pub(crate) fn seq_buf_pointer<'a>(&'a self) -> GPUPointer<'a, &'a [u64]> {
...
impl Buffer::ver {
    pub(crate) fn new(
        ...
        Ok(Buffer::ver {
            inner: Ref::try_new(Mutex::new(BufferInner::ver {
                info,
                ualloc,
                blocks: Vec::new(),
                max_blocks,
    ...
    pub(crate) fn new_scene(
        ...
        let scene_inner = box_in_place!(buffer::Scene::ver {
            ...
            stats: alloc.shared.new(Default::default(), |_inner| {
                buffer::raw::Stats {
