...
pub(crate) struct Guard<T: SlotItem> {
    ...
    changed: bool,
    token: SlotToken,
    alloc: Ref<SlotAllocatorOuter<T>>,
}
...
pub(crate) struct SlotAllocator<T: SlotItem>(Ref<SlotAllocatorOuter<T>>);
...
impl<T: SlotItem> SlotAllocator<T> {
    pub(crate) fn new(
        ...
        let mut alloc = Pin::from(UniqueRef::try_new(SlotAllocatorOuter {
