...
impl ObjectRef {
    ...
    pub(crate) fn map_into(&mut self, context: &crate::mmu::VM) -> Result<usize> {
        let mut mapping = self.gem.p.mapping.lock();
        if mapping.is_some() {
            Err(EBUSY)
        } else {
            let sgt = self.gem.sg_table()?;
            let new_mapping = context.map(self.gem.size(), &mut sgt.iter())?;
    ...
    pub(crate) fn map_into_range(&mut self, context: &crate::mmu::VM, start: u64, end: u64, alignment: u64, prot: u32) -> Result<usize> {
        let mut mapping = self.gem.p.mapping.lock();
        if mapping.is_some() {
            Err(EBUSY)
        } else {
            let sgt = self.gem.sg_table()?;
            let new_mapping = context.map_in_range(self.gem.size(), &mut sgt.iter(), alignment, start, end, prot)?;
